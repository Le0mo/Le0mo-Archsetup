function f
    ...
    # 修复：使用 count 和 test 来正确判断文件是否存在
    set -l FILES
    if count $CACHE_DIR/*.jpg >/dev/null 2>&1
    ...
    if test "$NUM_FILES" -gt 0
        ...
        # 检查文件是否真的存在
        if test -f "$SELECTED_IMG"
            # 后台补货
            background_job >/dev/null 2>&1
        else
            # 文件不存在，重置 SELECTED_IMG
            set SELECTED_IMG ""
        end
    end
    
    # 如果没有选中图片，尝试下载一张
    if test -z "$SELECTED_IMG"
    ...
    if test -n "$SELECTED_IMG"; and test -f "$SELECTED_IMG"
        ...
        # 检查 used 目录并清理旧图
        set -l used_files
        if count $USED_DIR/*.jpg >/dev/null 2>&1
            set used_files $USED_DIR/*.jpg
        end
        ...
        if test "$used_count" -gt "$MAX_USED_LIMIT"
            ...
            if count $USED_DIR/*.jpg >/dev/null 2>&1
                # 按时间排序，保留最新的 MAX_USED_LIMIT 张
                set files_to_delete (ls -t $USED_DIR/*.jpg 2>/dev/null | tail -n +$skip_lines)
            end
